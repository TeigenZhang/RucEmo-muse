...
from .networks.lstm_encoder import LSTM_official_Encoder, BiLSTMEncoder

class FcFusionSlideModel(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt, logger=None):
        ...
        if opt.bidirection:
            ...
            self.net_seq = LSTM_official_Encoder(opt.hidden_size, opt.hidden_size)
            self.hidden_mul = 1 
    ...
    def set_input(self, input, load_label=True):
        ...
        if self.normalize:
            self.feature = self.normalize_feature(self.feature)
        if load_label:
    ...
    def run(self):
        ...
        prediction, _ = self.forward_step(self.feature)
        # backward
        if self.isTrain:
    ...
    def forward_step(self, input):
        ...
        hidden, (h, c) = self.net_seq(fusion)
